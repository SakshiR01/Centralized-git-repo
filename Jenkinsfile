       ...
       stage('Build_image') {
			...
			container("${env.TRIVY_CONTAINER}") {
		        ...
		        //   sh 'sed -i -e "s/SERVICE/$SERVICE/g" Dockerfile deployment-type.yaml' 
                  sh 'sed -i -e "s/SERVICE/$SERVICE/g" -e "s/PORT/$PORT/g"  -e "s/REGISTRY/$REGISTRY/g" -e "s/IMAGE/$IMAGE/g" -e "s/COPY_CMD/$COPY_CMD/g" -e "s/WORKDIR_CMD/$WORKDIR_CMD/g" Dockerfile deployment-beta.yaml' 
		          sh 'cat Dockerfile'	  
